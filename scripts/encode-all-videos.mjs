...
async function encodeOne(src, destMp4) {
  // Full length, high quality web delivery (~original look, streamable)
  // Keep native resolution up to 1920 wide; CRF 17 ≈ visually lossless
  await runFfmpeg([
    "-y",
    "-i",
    src,
...
async function posterFromVideo(src, destBase) {
  ...
  await pipe.clone().jpeg({ quality: 90, mozjpeg: true }).toFile(`${destBase}.jpg`);
...
async function main() {
  ...
  console.log(`Encoding ${files.length} full-length videos (high quality)...`);
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const posterBase = path.join(posterDir, `film-${String(i + 1).padStart(2, "0")}`);

    console.log(`\n[${i + 1}/${files.length}] ${file}`);
    try {
      // Skip re-encode if dest already exists and is reasonably large
      let needEncode = true;
      try {
        const st = await fs.stat(dest);
        if (st.size > 5_000_000) {
          needEncode = false;
          console.log("  clip exists, skipping encode");
        }
      } catch {}

      if (needEncode) {
        console.log("  encoding full video...");
      ...
      await posterFromVideo(src, posterBase);
      ...
      films.push({
        ...
        poster: `/media/films/film-${String(i + 1).padStart(2, "0")}.webp`,
        posterAvif: `/media/films/film-${String(i + 1).padStart(2, "0")}.avif`,
        category,
  ...
  console.log(`\nManifest updated with ${films.length} full films.`);
}
